...
@section('content')
<div class="flex justify-end mb-4"><a href="{{ route('teacher.certchain.certificates.download', $certificate) }}" class="bg-primary text-on-primary px-4 py-2 rounded-xl font-bold hover:scale-[1.02] transition-all text-sm">â¬‡ Download PDF</a>
@if(!$certificate->email_sent)
<form method="POST" action="{{ route('teacher.certchain.certificates.email', $certificate) }}" class="inline">
    @csrf
    <button class="bg-amber-500 text-white px-4 py-2 rounded-xl font-bold hover:scale-[1.02] transition-all text-sm ml-2">ðŸ“§ Send Email</button>
</form>
@endif</div>
<div class="grid lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-5">
        ...
        <div class="bg-surface-container rounded-3xl border border-outline-variant/10 shadow-lg p-6">
            <h3 class="font-semibold text-on-surface font-bold mb-4">ðŸŽ“ Student Information</h3>
            <div class="grid md:grid-cols-2 gap-y-4 gap-x-6 text-sm">
                ...
                $fields = [
                    ...
                    'Branch'            => $certificate->student_branch ?? 'â€”',
                    'Year'              => $certificate->student_year ?? 'â€”',
                    'Achievement'       => $certificate->achievement,
                    'Issued Date'       => $certificate->issued_date?->format('d M Y'),
                    'Issued By'         => $certificate->issuer->name ?? 'â€”',
                ];
                @endphp
        ...
        {{-- Event Details --}}
        <div class="bg-surface-container rounded-3xl border border-outline-variant/10 shadow-lg p-6">
            <h3 class="font-semibold text-on-surface font-bold mb-4">ðŸ“… Event Information</h3>
            <div class="grid md:grid-cols-2 gap-y-4 text-sm">
                ...
                <div><p class="text-xs text-on-surface-variant opacity-70 mb-0.5">Venue</p><p class="font-medium">{{ $certificate->event->venue ?? 'â€”' }}</p></div>
            </div>
        </div>
    </div>
