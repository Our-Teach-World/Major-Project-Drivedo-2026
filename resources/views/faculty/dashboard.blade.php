...
<a href="{{ route('teacher.certchain.certificates.create') }}" class="btn-gold text-sm">+ Issue Certificate</a>
@endsection
...
<div class="grid lg:grid-cols-2 gap-6">
    ...
    <div class="card p-5">
        ...
        <div class="space-y-3">
            ...
            <a href="{{ route('teacher.certchain.certificates.show', $cert) }}" class="flex items-center justify-between py-2 border-b border-gray-50 last:border-0 hover:bg-gray-50 -mx-2 px-2 rounded">
                <div>
            ...
            <p class="text-sm text-gray-400 text-center py-6">No certificates issued yet.<br>
                <a href="{{ route('teacher.certchain.certificates.create') }}" class="text-blue-600 hover:underline">Issue your first one →</a>
            </p>
            @endforelse
        </div>
    </div>
    ...
    <div class="card p-5">
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('teacher.certchain.events.create') }}" class="text-xs text-blue-600 hover:underline">+ New event</a>
        </div>
        <div class="space-y-3">
...
<div class="mt-6 card p-5">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
        <a href="{{ route('teacher.certchain.events.create') }}" class="flex flex-col items-center gap-2 p-4 bg-blue-50 rounded-xl hover:bg-blue-100 transition text-center">
            <span class="text-2xl">📅</span>
            <span class="text-xs font-medium text-blue-800">Create Event</span>
        </a>
        <a href="{{ route('teacher.certchain.certificates.create') }}" class="flex flex-col items-center gap-2 p-4 bg-green-50 rounded-xl hover:bg-green-100 transition text-center">
            <span class="text-2xl">📜</span>
            <span class="text-xs font-medium text-green-800">Issue Certificate</span>
        </a>
        <a href="{{ route('teacher.certchain.certificates.bulk') }}" class="flex flex-col items-center gap-2 p-4 bg-yellow-50 rounded-xl hover:bg-yellow-100 transition text-center">
            <span class="text-2xl">📦</span>
